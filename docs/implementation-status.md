...
| Debate / 飞书投影 | 有界 Debate 领域模型已实现，含轮次、单 Agent 和总消息上限；新增持久化房间、消息和关闭 API，并校验上下文版本与重复消息；Debate Brief 可携带受控 Context Pack；新增内部模型池的有界回合编排和 `run` API；新增带 hash/幂等键、失败重试状态的渠道无关 Projection outbox 与 HTTP sink | 具体飞书/Hermes adapter、独立 Moderator/Adjudicator 和生产投影权限仍需接入验收 |
