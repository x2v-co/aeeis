...
function assertWithinTimeout() {
  if (Date.now() - startedAt > timeoutMs) throw new Error(`full local smoke test timed out after ${timeoutMs}ms`);
}
...
async function waitForReady() {
  while (true) {
    ...
    try {
      ...
      // full local services may still be starting. The bounded loop is the timeout.
...
if (health.service !== 'aeeis-agent' || health.protocol !== 'aeeis-health/1') {
  throw new Error(`full local smoke target is not an AEEIS API (check AEEIS_BASE_URL; got ${JSON.stringify(health)})`);
}
const status = await request('/api/status');
if (status.runner !== 'LocalDispatcher' || status.dispatcherHealth?.ready !== true) {
  throw new Error(`Full local smoke requires LocalDispatcher readiness, got ${JSON.stringify({ runner: status.runner, dispatcherHealth: status.dispatcherHealth })}`);
}
if (status.modelRouting !== 'catalog' || !String(status.modelHealth?.detail ?? '').includes('catalog selected compose-fixture/aeeis-fixture/1')) {
  throw new Error(`full local smoke requires Planprice catalog routing, got ${JSON.stringify({ modelRouting: status.modelRouting, modelHealth: status.modelHealth })}`);
}
if (status.executionProfile !== 'fixture') throw new Error('full local must identify itself as a fixture demonstration');
if (status.skillGovernanceConfigured !== true || status.skillGovernanceHealth?.ready !== true || status.rsiProposalSynthesisConfigured !== true) {
  throw new Error(`full local smoke requires OwnHow health and RSI proposal synthesis wiring, got ${JSON.stringify({ skillGovernanceConfigured: status.skillGovernanceConfigured, skillGovernanceHealth: status.skillGovernanceHealth, rsiProposalSynthesisConfigured: status.rsiProposalSynthesisConfigured })}`);
}
...
const synthesisRunRef = await request('/api/runs', {
  ...
  body: JSON.stringify({
    ...
    materials: [{ title: 'RSI smoke evidence', source: 'full-local-rsi', content: 'The controlled review signal is evidence for this development smoke.' }],
  }),
});
...
// Local mode uses the in-process reminder pump; reminder timer durability is
// covered by the dedicated Compose smoke.
...
const triggerPolicy = await request('/api/collaborations/triggers/policies', {
  method: 'POST',
  body: JSON.stringify({
    id: `policy.full-local.${Date.now()}`,
    name: 'full local trigger competition',
    enabled: true,
...
const triggerEvent = {
  eventId: `review:full-local:${Date.now()}`,
  eventType: 'review.completed',
  source: 'system',
  taskId: 'task.full-local.trigger',
  contextVersion: 'ctx.full-local.trigger',
  goal: 'Choose the bounded full local smoke result',
  risk: 'high',
...
const debatePolicy = await request('/api/collaborations/triggers/policies', {
  method: 'POST',
  body: JSON.stringify({
    id: `policy.full-local.debate.${Date.now()}`,
    name: 'full local trigger debate',
    enabled: true,
...
const debateEvent = {
  eventId: `task:full-local-debate:${Date.now()}`,
  eventType: 'task.failed',
  source: 'system',
  taskId: 'task.full-local.debate',
  contextVersion: 'ctx.full-local.debate',
...
const goal = await request('/api/goals', {
  method: 'POST',
  body: JSON.stringify({ title: `full local smoke ${new Date().toISOString()}` }),
});
const plan = await request(`/api/goals/${goal.id}/plans`, {
  method: 'POST',
  body: JSON.stringify({ nodes: [
    { id: 'inspect', title: 'Inspect smoke evidence', instruction: 'Inspect the full local smoke evidence.' },
    { id: 'deliver', title: 'Deliver smoke result', instruction: 'Deliver the verified full local smoke result.', dependsOn: ['inspect'] },
  ] }),
});

const scheduled = await request(`/api/plans/${plan.id}/schedule`, { method: 'POST', body: JSON.stringify({
  materials: [{ title: 'Smoke source', source: 'full-local-fixture', content: 'AEEIS full local smoke evidence.' }],
}) });
...
async function approveRun(runId, options = {}) {
  const run = await waitForRun(runId, ['needs_approval', 'succeeded', 'failed', 'cancelled'], options);
...
const pulseGoal = await request('/api/goals', {
  method: 'POST',
  body: JSON.stringify({ title: `full local Project Pulse ${new Date().toISOString()}` }),
});
...
const firstRun = await approveRun(firstDispatch.runId, { allowQueuedReservation: true });
...
const secondRun = await approveRun(secondDispatch.runId, { allowQueuedReservation: true });
...
// Verify the external Agent boundary through the real full local network. The
...
const runEventPolicy = await request('/api/collaborations/triggers/policies', {
  method: 'POST',
  body: JSON.stringify({
    id: `policy.full-local.run-event.${Date.now()}`,
    name: 'full local Run review collaboration',
    enabled: true,
    eventTypes: ['review.completed'],
    sources: ['system'],
    action: {
...
const externalRun = await request('/api/runs', {
  ...
  body: JSON.stringify({
    ...
    materials: [{ title: 'External smoke source', source: 'full-local-fixture-agent', content: 'The fixture Agent must preserve this evidence boundary.' }],
...
// Verify the RSI boundary through the real full local evaluator. The evaluator
...
if (!activePrompt) {
  // A fresh data directory has only the immutable prompt/1 baseline and no
  // active release pointer. Establish a real, evaluated release before
  // exercising the production traffic boundary.
  const baselineVersion = `prompt/full-local-baseline-${Date.now()}`;
  const baseline = await request('/api/evolution/candidates', {
    method: 'POST',
    body: JSON.stringify({
      target: 'prompt', baseVersion: 'prompt/1', proposedVersion: baselineVersion,
      change: 'Preserve evidence citations in the local smoke baseline.',
      sourceReceiptRefs: ['receipt.full-local.baseline'], reason: 'Establish a canary base release', risk: 'low',
    }),
  });
  const evaluatedBaseline = await request(`/api/evolution/candidates/${baseline.id}/evaluate-suite`, {
    method: 'POST', body: JSON.stringify({ suite: {
      replay: [{ id: 'replay.full-local-baseline', input: { goal: 'baseline' } }],
      holdout: [{ id: 'holdout.full-local-baseline', input: { goal: 'baseline' } }],
      safety: [{ id: 'safety.full-local-baseline', input: { goal: 'safe' } }],
    } }),
  });
  ...
  await request(`/api/evolution/candidates/${baseline.id}/approve`, { method: 'POST', body: JSON.stringify({ approvalRef: 'full-local-baseline-approval' }) });
  await request(`/api/evolution/candidates/${baseline.id}/promote`, { method: 'POST', body: JSON.stringify({}) });
  await request(`/api/evolution/candidates/${baseline.id}/activate`, { method: 'POST', body: JSON.stringify({ activationRef: 'full-local-baseline-activation' }) });
...
const rsiProposedVersion = `prompt/full-local-smoke-${Date.now()}`;
const candidate = await request('/api/evolution/candidates', {
  ...
  body: JSON.stringify({
    ...
    sourceReceiptRefs: ['receipt.full-local.rsi'],
    reason: 'full local RSI evaluator smoke',
    risk: 'low',
  }),
});
const evaluatedCandidate = await request(`/api/evolution/candidates/${candidate.id}/evaluate-suite`, {
  ...
  body: JSON.stringify({
    suite: {
      replay: [{ id: 'replay.full-local', input: { goal: 'full-local-rsi' } }],
      holdout: [{ id: 'holdout.full-local', input: { goal: 'full-local-rsi' } }],
      safety: [{ id: 'safety.full-local', input: { goal: 'safe' } }],
    },
  }),
});
...
const approvedCandidate = await request(`/api/evolution/candidates/${candidate.id}/approve`, {
  method: 'POST',
  body: JSON.stringify({ approvalRef: 'full-local-rsi-approval' }),
});
...
const trafficRoute = await request(`/api/evolution/candidates/${candidate.id}/start-traffic`, {
  method: 'POST',
  body: JSON.stringify({ percentage: 2500, rolloutRef: 'full-local-rsi-traffic-start' }),
});
...
const trafficRunRef = await request('/api/runs', {
  method: 'POST',
  body: JSON.stringify({ goal: 'Freeze the full local production traffic decision' }),
});
...
const observedTraffic = await request(`/api/evolution/candidates/${candidate.id}/record-traffic`, {
  method: 'POST',
  body: JSON.stringify({ id: 'traffic.full-local.observation', passed: true, score: 0.98, evidenceRefs: ['metric.full-local.acceptance', 'metric.full-local.latency'] }),
});
if (observedTraffic.observations?.length !== 1 || observedTraffic.observations[0]?.id !== 'traffic.full-local.observation') {
...
const stoppedTraffic = await request(`/api/evolution/candidates/${candidate.id}/stop-traffic`, {
  method: 'POST',
  body: JSON.stringify({ reason: 'full local smoke completed the traffic observation window' }),
});
...
const activatedCandidate = await request(`/api/evolution/candidates/${candidate.id}/activate`, {
  method: 'POST',
  body: JSON.stringify({ activationRef: 'full-local-rsi-activation' }),
});
