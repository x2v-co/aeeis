...
describe('AEEIS HTTP boundary', () => {
  ...
  it('exposes durable competition and debate collaboration endpoints', async () => {
    ...
    await app.inject({ method: 'POST', url: `/api/collaborations/competitions/${competition.id}/score`, payload: { evaluatorAgentId: 'agent.eval', score: { agentId: 'agent.one', score: 0.8, accepted: true, reasons: [], evidenceRefs: [] } } });
    expect((await app.inject({ method: 'POST', url: `/api/collaborations/competitions/${competition.id}/score`, payload: { evaluatorAgentId: 'agent.eval', score: { agentId: 'agent.two', score: 0.5, accepted: true, reasons: [], evidenceRefs: [] } } })).json().status).toBe('completed');
